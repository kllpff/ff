...
request()->url()                    // Текущий абсолютный URL

response($content, 200)             // Создать ответ
response()->json($data)             // JSON ответ
redirect('/url')                    // Перенаправление
route('name', ['id' => 1])         // URL маршрута
url('/path')                        // Абсолютный URL (на основе APP_URL)
```

Рекомендации для API:

- Предпочитайте `response()->json($data)`, чтобы гарантировать корректные заголовки и кодировку.
- Избегайте ручного `echo` JSON или самостоятельной установки заголовков.
- Возврат массивов/объектов из API-контроллеров может автоматически конвертироваться в JSON, но явное использование `response()->json(...)` обеспечивает единообразие и контроль над ответами.
...
// Шифрование - рекомендуемый подход (внедрение зависимостей)
$encryptor = app('encrypt');
$encrypted = $encryptor->encrypt('данные');
$decrypted = $encryptor->decrypt($encrypted);

// Альтернатива: статические методы (если доступны)
// Encrypt::encrypt('данные');
